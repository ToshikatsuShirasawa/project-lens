...
function OrgHomeContent() {
  ...
  const [lastVisitedProjectId, setLastVisitedProjectIdState] = useState<string | null>(null)
  ...
  const loadProjects = useCallback(async (orgId: string) => {
    ...
    setProjects(null)
    try {
      const res = await fetch(`/api/projects?organizationId=${encodeURIComponent(orgId)}`, {
        cache: 'no-store',
        credentials: 'include',
      })
      const body: unknown = await res.json().catch(() => null)
  ...
  useEffect(() => {
    if (!organizationId || !currentOrg) return
    void loadProjects(organizationId)
  }, [organizationId, currentOrg, loadProjects])

  useEffect(() => {
    setLastVisitedProjectIdState(organizationId ? getLastVisitedProjectId(organizationId) : null)
  }, [organizationId])
  ...
  const listUrl = `/o/${encodeURIComponent(orgId)}/projects`
  const recentProject = projects?.find((p) => p.id === lastVisitedProjectId) ?? null
